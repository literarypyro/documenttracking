...
//LEFT SIDE
	styleCellArea(setRange("c3","g3"),"true","true",$ExWs,$excel);
	styleCellArea(setRange("c4","g4"),"true","true",$ExWs,$excel);
	...
	$rr[0]=strtoupper($rr[0]);
	$rr[1]=strtoupper($rr[1]);
	...
	if($nm>0){
		...
		$excel->getActiveSheet()->getStyle($rr[0].":".$rr[1])->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$excel->getActiveSheet()->getStyle($rr[0].":".$rr[1])->getAlignment()->setWrapText(true);
		$excel->getActiveSheet()->getStyle($rr[0].":".$rr[1])->getFont()->setSize(12); 	
//		setHeadingArea("minor",$rr,"true",$excel,$ExWs);
	...
	if($nm>0){
		...
		for($m=0;$m<$nm;$m++){
			...
			addContent($rr,$excel,getOriginatingOffice($db,$row['originating_office']),"false",$ExWs);
			setCellArea($rr,$excel,$ExWs,$excel);
			styleCellArea(setRange($rr[0],$rr[1]),"false","true",$ExWs,$excel);
