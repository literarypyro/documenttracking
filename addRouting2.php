...
* or to all offices.   This version of the second part allows for the attaching of
* soft copy or scanned document within a routing action-destination
*
...
<!-- Attaching and using Twitter Bootstrap -->
...
thead th {
	...
	<div align=left >This page will reload to the main page after 7 mins.</div>
	...
	$rs=getRoutingTargets($db,$_SESSION['routingID']); //retrieved from functions/routing process	
	$nm=$rs->num_rows;
	for($i=0;$i<$nm;$i++){
		$row=$rs->fetch_assoc();
		...
		if($row['to_name']=="OTHER"){
			...
			$row2=getOfficer($db,$row['to_name']);  //retrieved from functions/form functions
		...
		$action=getAction($db,$row['action_id']); //retrieved from functions/routing process
	...
	<input class='form-control' type=checkbox name='sendToAll' value='on' onclick='disableForm(this)' />
	</div>
	<div class='form-group'>
	SEND TO ALL OFFICES
	</div>
	...
	<?php
		...
		retrieveOfficerListHTML($db,"to_name"); //retrieved from functions/form functions
	?>	
	</td>
	</tr>
	...
	<td>Position</td>
	<td>
	<input type='text' class='form-control' id='alterPosition' name='alterPosition' />
	</td>
	</tr>	
	...
	<td><textarea class='form-control' name='remarks' cols=70 rows=5></textarea></td>
	</tr>
	<tr><td>Upload File (optional)</td><td><input class='form-control' type='file' name='routing_file' /></td></tr>
	<tr>
