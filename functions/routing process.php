...
function addNewRouting($db,$referenceNumber,$date,$office,$officerMark){
	...
	$sql="insert into document_routing(reference_no,from_name,request_date,from_office,alter_from,alter_position) values
	('".$referenceNumber."','".$officer."','".$date."','".$office."','".$alter_officer."','".$alter_position."')";
	$rs=$db->query($sql);
...
function addRoutingStatus($setDb,$routing){
	...
	(destination_office,to_name,action_id, status, routing_id, remarks,alter_to,alter_position)
	values
	(\"".$destination."\",\"".$to_name."\",\"".$action_id."\",\"".$status."\",\"".$routing_id."\",\"".$remarks."\",\"".$alter_to."\",\"".$alter_position."\")";
	$rs=$setDb->query($sql);
